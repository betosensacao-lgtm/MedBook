  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Schedule & Completed Appointments</h1>
          <p className="text-gray-500 text-sm mt-1">
            Appointments and triages captured in real time via Web Chat and WhatsApp
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-gray-100 bg-gray-50/50 flex items-center justify-between">
          <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">
            {apptList.length} appointment(s) in the system
          </span>
        ...
        {apptList.length === 0 ? (
          <div className="p-12 text-center text-gray-500 text-sm">
            ...
            <p className="text-xs text-gray-400 mt-1">Appointments confirmed via the AI chat will appear here in real time.</p>
          </div>
        ) : (
            ...
            {apptList.map((event) => (
              <div key={event.id} className="p-5 hover:bg-gray-50 transition-colors flex items-center justify-between">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h3 className="font-semibold text-gray-900 text-sm">{event.notes || "Medical Appointment"}</h3>
                    <div className="flex items-center gap-4 mt-1.5 text-xs text-gray-500">
                      <span className="flex items-center gap-1 font-medium text-gray-700">
                        <User className="w-3.5 h-3.5 text-teal-600" />
                        {event.patientName || "Patient"} {event.patientPhone ? `(${event.patientPhone})` : ""}
                      </span>
                      <span className="flex items-center gap-1">
                        <Clock className="w-3.5 h-3.5 text-gray-400" />
                        {event.date} at {event.startTime}
                      </span>
                      <span className="text-gray-400">| {event.professionalName || "General Practice"}</span>
                    </div>
                  </div>
                </div>
      ...
      {triages.length > 0 && (
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
          <div className="p-4 border-b border-gray-100 bg-gray-50/50 flex items-center justify-between">
            <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">
              {triages.length} Pre-Anamnesis & Triages Recorded
            </span>
          </div>
          <div className="divide-y divide-gray-100">
            {triages.map((t) => (
              <div key={t.id} className="p-4 hover:bg-gray-50 flex items-center justify-between text-sm">
                <div>
                  ...
                  <p className="text-xs text-gray-600 mt-0.5"><span className="font-medium text-teal-700">Symptoms:</span> {t.mainSymptom || "General"}</p>
                </div>
